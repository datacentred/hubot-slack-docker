    ...
    tixcurlhack.exec "curl -XGET '#{hostname}:9200/_all/_search?' -H 'Content-Type: application/json' -d'#{query}' ", (error, stdout, stderr) ->
      if error
        ...
        if counter > 10
          ...
          robot.messageRoom '@matt', "The number of vrouter failovers in the last ten minutes is high. Check the support channel for details."
          robot.messageRoom '@bart', "The number of vrouter failovers in the last ten minutes is high. Check the support channel for details."
